...
function Show(props) {
    ...
    const newFav = async (event) => {
        try {
          const response = await axios.put((process.env.API_URL || 'http://localhost:3001') + '/users/favorites', {
            newFav: {
                id: id,
                title: comics.title,
    ...
    useEffect(() => {
        async function fetchID() {
            ...
            const response = await axios.get((process.env.API_URL || `http://localhost:3001`) + `/marvel/comics/${id}`)
